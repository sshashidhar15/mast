...
const prefix = 'Migration WEB-1201 ICMarkets -> ICMarkets (SC) for FSA 2 '
    ...
    redis((_, db) =>
        Promise.all([
            ...
            .then(([asic, fsa]) => {
                ...
                langs.forEach(lang => {
                    fsa[lang] = msgpack.decode(asic[lang])
                    const paths_to_updated_keys = deep_replace(fsa[lang], [], [])
                    ...
                    paths_to_updated_keys.forEach(path => {
                        ...
                        const value = path.reduce((tree, key) => tree[key], fsa[lang])

                        short_path.reduce((tree, key) => tree[key], fsa[lang])[k] = value
                    })
